...
import {serviceSave, serviceLoad} from "@/service/cadastros/mecanismo_resistencia_microorganismo";
...
const verivicarCodigo = async (codigo = null) => {
    const statusCodigo = await verificarCodigo({
        codigo: codigo,
        tabela: 'mecanismo_resistencia_microorganismo'
    });
...
const handleBack = () => {
    router.push({name: 'MecanismoResistenciaMicroorganismo-List'});
};

</script>

<template>
    <card-formulario title="Cadastro Mecanismo de Resistência"
