...
import {serviceSave, serviceLoad} from "@/service/cadastros/perfil_resistencia_microorganismo";
...
const verivicarCodigo = async (codigo = null) => {
    const statusCodigo = await verificarCodigo({
        codigo: codigo,
        tabela: 'perfil_resistencia_microorganismo'
    });
...
const handleBack = () => {
    router.push({name: 'PerfilResistenciaMicroorganismo-List'});
};

</script>

<template>
    <card-formulario title="Cadastro Perfil de Resistência"
