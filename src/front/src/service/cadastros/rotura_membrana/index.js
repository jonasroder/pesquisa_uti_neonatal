...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/rotura_membrana/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/rotura_membrana/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/rotura_membrana/list`);
		return response.data;
	} catch (e) {
