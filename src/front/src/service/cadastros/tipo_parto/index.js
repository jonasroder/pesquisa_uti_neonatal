...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/tipo_parto/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/tipo_parto/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/tipo_parto/list`);
		return response.data;
	} catch (e) {
