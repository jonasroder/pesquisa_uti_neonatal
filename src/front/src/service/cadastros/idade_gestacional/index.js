...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/idade_gestacional/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/idade_gestacional/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/idade_gestacional/list`);
		return response.data;
	} catch (e) {
