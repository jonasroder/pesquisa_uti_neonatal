...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/causa_obito/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/causa_obito/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/causa_obito/list`);
		return response.data;
	} catch (e) {
