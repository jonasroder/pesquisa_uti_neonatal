...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/classificacao_microorganismo/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/classificacao_microorganismo/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/classificacao_microorganismo/list`);
		return response.data;
	} catch (e) {
