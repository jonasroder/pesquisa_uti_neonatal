...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/peso_nascimento/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/peso_nascimento/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/peso_nascimento/list`);
		return response.data;
	} catch (e) {
