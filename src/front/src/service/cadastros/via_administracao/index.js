...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/via_administracao/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/via_administracao/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/via_administracao/list`);
		return response.data;
	} catch (e) {
