...
export const serviceLoad = async (id) => {
	try {
		const response = await axios.get(`/api/sitio_cirurgia/load/${id}`);
...
export const serviceSave = async (data) => {
	try {
		let res = await axios.post('/api/sitio_cirurgia/save', data);
...
export const serviceList = async () => {
	try {
		const response = await axios.get(`/api/sitio_cirurgia/list`);
		return response.data;
	} catch (e) {
